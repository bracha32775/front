...
EnhancedTableToolbar.propTypes = {
  ...
  onExportWord: PropTypes.func,
};

function EnhancedTable({ onDeleteStudent, onUpdateStudent, isUpdating }) {
  const dispatch = useDispatch();
  const students = useSelector(state => state.students.students || []);
  ...
  return (
    <CacheProvider value={cacheRtl}>
      <ThemeProvider theme={rtlTheme}>
        <Box sx={{ width: '100%' }} dir="rtl">
          <Paper sx={{ width: '100%', mb: 2 }}>
            <EnhancedTableToolbar onExportExcel={handleExportExcel} onExportWord={() => {}} />
            <TableContainer>
              {isUpdating && (
                <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 2, height: '4px', overflow: 'hidden' }}>
                  <Box sx={{ width: '100%', height: '100%', bgcolor: 'primary.main', animation: 'loading 2s infinite' }} />
                ...
                <TableBody>
                  {visibleRows.map((row) => (
                    <TableRow 
                      ...
                      tabIndex={-1} 
                      key={row.idOfStudent} 
                      onClick={() => onUpdateStudent && onUpdateStudent(row)} 
                      sx={{ cursor: onUpdateStudent ? 'pointer' : 'default' }}
                    >
                      <TableCell align="center">{row.nameOfStudent}</TableCell>
                      <TableCell align="center">{row.addressOfStudent}</TableCell>
                      <TableCell align="center">{row.phoneOfStudent}</TableCell>
...
// חשוב מאוד: ייצוא ברירת מחדל של הקומפוננטה
