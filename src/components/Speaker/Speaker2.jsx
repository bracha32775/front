...
import { AddSpeaker } from '../Add/AddSpeaker/AddSpeaker';
import { deleteSpeakerThunk } from '../../redux/Thunks/deleteSpeakerThunk';
// import { getBySpeakerNameThunk } from '../../redux/Thunks/getBySpeakerNameThunk'; // לא נחוץ יותר
import { setFlagSpeakerSlice } from '../../redux/Slices/speakersSlice';
import * as XLSX from 'xlsx';
import FileDownloadIcon from '@mui/icons-material/FileDownload';
import {
...
export const Speaker2 = () => {
    ...
    const filteredSpeakers = speakers.filter(speaker =>
        speaker.nameOfSpeaker?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        speaker.specializationOfSpeaker?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        speaker.addressOfSpeaker?.toLowerCase().includes(searchTerm.toLowerCase()) || // הוספת חיפוש לפי כתובת
        speaker.emailOfSpeaker?.toLowerCase().includes(searchTerm.toLowerCase()) // הוספת חיפוש לפי אימייל
    );
    ...
    // בדיקה האם המרצה פעיל (לפי שדה הסטטוס)
    const isSpeakerActive = (speaker) => {
        return speaker.status; // מניח ששדה status הוא בוליאני (true/false)
    };
    ...
    const handleDeleteSpeaker = async () => {
        ...
        setLoading(true);
        try {
            // מציאת המרצה לפי השם שנבחר מהרשימה הקיימת בסטייט
            const speakerToDelete = speakers.find(s => s.nameOfSpeaker === deleteSpeakerName);

            if (speakerToDelete) {
                // שליחת ה-ID של המרצה ל-thunk למחיקה (שינוי סטטוס)
                // שימוש ב-unwrap לטיפול בשגיאות מה-thunk
                // await dispatch(deleteSpeakerThunk(speakerToDelete.idOfSpeaker)).unwrap();
                // setSnackbar({
                    await dispatch(deleteSpeakerThunk(speakerToDelete.idOfSpeaker)).unwrap();
                    setSnackbar({
                        open: true,
                        message: `המרצה ${deleteSpeakerName} הועברה לארכיון בהצלחה`, // עדכון הודעה
                        severity: 'success'
                    });
                    setOpenDelete(false);
                    setDeleteSpeakerName("");
                    // רענון רשימת המרצים לאחר המחיקה
                    dispatch(getSpeakersThunk());
                } else {
                     setSnackbar({
                        open: true,
                        message: `שגיאה: המרצה ${deleteSpeakerName} לא נמצא ברשימה`,
                        severity: 'error'
                    });
                }
            } catch (error) {
                ...
                setSnackbar({
                    open: true,
                    message: `שגיאה במחיקת המרצה: ${error.message || 'שגיאה לא ידועה'}`,
                    severity: 'error'
                });
            } finally {
                setLoading(false);
                    ...
                    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
                        ...
                        >
                            ...
                            <SearchField
                                fullWidth
                                variant="outlined"
                                placeholder="חיפוש לפי שם, תחום, כתובת או אימייל..." // עדכון פלייסהולדר
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                sx={{ mb: 3 }}
                                InputProps={{
                            ...
                            <Box sx={{ mt: 2 }}>
                                <FormControl fullWidth>
                                    ...
                                    <Select
                                        ...
                                        label="בחר מרצה למחיקה"
                                    >
                                        {/* מציג רק מרצים פעילים לבחירה למחיקה */}
                                        {speakers.filter(s => s.status).map((speaker) => (
                                            <MenuItem key={speaker.idOfSpeaker} value={speaker.nameOfSpeaker}>
                                                {speaker.nameOfSpeaker}
                                            </MenuItem>
                                        ))}
